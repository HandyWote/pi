...
function makeDefaultModel(discovered: DiscoveredProfileModel): UserModel {
	return {
		...discovered,
		...
		cost: zeroCost(),
		group: inferModelDisplayGroup(discovered.id, discovered.name),
		available: true,
	};
}

function enrichOne(discovered: DiscoveredProfileModel, modelsDev: ModelsDevData): UserModel {
	...
	if (matches.length === 0) return makeDefaultModel(discovered);
	...
	return {
		...
		group: inferModelDisplayGroup(discovered.id, best.name ?? discovered.name),
		available: true,
	};
}
...
export async function enrichWithModelsDev(models: DiscoveredProfileModel[]): Promise<UserModel[]> {
	let catalog: ModelsDevData;
	try {
		catalog = await fetchModelsDevCatalog();
	} catch {
		return models.map((model) => makeDefaultModel(model));
	}
	return models.map((model) => enrichOne(model, catalog));
}
