...
function authHeaders(apiKey: string, style: ProfileAuthStyle): Record<string, string> {
	if (style === "anthropic") {
		return {
...
function routeSupportsApi(catalog: Catalog, api: RegistryApi): boolean {
	if (catalog.availableApis.includes(api)) return true;
	if (catalog.models.some((model) => model.availableApis?.includes(api))) return true;
	return catalog.availableApis.length === 0 && catalog.models.every((model) => model.availableApis?.length === 0);
}
...
async function fetchCatalog(
	...
	const response = await fetchSameOrigin(
		fetchImpl,
		request.url,
		{ method: "GET", headers: authHeaders(resolveAuthApiKey(profile), request.authStyle) },
		timeoutMs,
		true,
...
async function verifyInferenceRoute(
	...
	try {
		response = await fetchSameOrigin(
			fetchImpl,
			route.inferenceUrl,
			{ method: "POST", headers: authHeaders(resolveAuthApiKey(profile), route.authStyle), body: "{}" },
			timeoutMs,
			false,
		);
	} catch (error) {
...
export async function discoverProfile(
	...
	for (const catalogSuccess of catalogResults) {
		...
		for (const route of candidateRoutes) {
			...
			} else if (verification.failure?.includes("authentication failed")) {
				// A successful authenticated catalog request is stronger evidence than a
				// deliberately incomplete inference request. Some gateways return 403 for
				// that probe even with a valid credential.
				verifiedRoutes.push(route);
				warnings.push(
					`${route.api}: inference probe was rejected after catalog authentication; route kept from catalog capability metadata`,
				);
			} else {
				failures.push({
					route: failureRoute(route),
