...
```bash
npm install @handy_wote/pi-ai
```

TypeBox exports are re-exported from `@handy_wote/pi-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@handy_wote/pi-ai';
import { builtinModels } from '@handy_wote/pi-ai/providers/all';
...
import { openaiProvider } from '@handy_wote/pi-ai/providers/openai';
import { openrouterProvider } from '@handy_wote/pi-ai/providers/openrouter';
import { amazonBedrockProvider } from '@handy_wote/pi-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@handy_wote/pi-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@handy_wote/pi-ai/providers/all';
...
Providers may have dynamic model lists (a local OpenAI-compatible server, a live OpenRouter listing). Reads stay sync; fetching is an explicit async verb:

```typescript
// getModels() returns the last-known list (empty before the first refresh)
await models.refresh('local-server');   // fetch one provider's list; rejects on failure
await models.refresh();                  // refresh all providers concurrently, best-effort
const fresh = models.getModel('local-server', 'qwen3-30b');
```
...
```typescript
import { createModels, type CredentialStore } from '@handy_wote/pi-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@handy_wote/pi-ai';
...
```typescript
import { validateToolCall, type Tool } from '@handy_wote/pi-ai';
...
```typescript
import { builtinImagesModels } from '@handy_wote/pi-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@handy_wote/pi-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@handy_wote/pi-ai';
...
const localServer = createProvider({
  id: 'local-server',
...
models.setProvider(localServer);
...
```typescript
import { stream } from '@handy_wote/pi-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@handy_wote/pi-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@handy_wote/pi-ai';
import { anthropicProvider } from '@handy_wote/pi-ai/providers/anthropic';
import { openaiProvider } from '@handy_wote/pi-ai/providers/openai';
import { googleProvider } from '@handy_wote/pi-ai/providers/google';
...
- `@handy_wote/pi-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@handy_wote/pi-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@handy_wote/pi-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@handy_wote/pi-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@handy_wote/pi-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@handy_wote/pi-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@handy_wote/pi-ai/bedrock-provider';
...
```bash
npx @handy_wote/pi-ai login              # interactive provider selection
npx @handy_wote/pi-ai login anthropic    # login to specific provider
npx @handy_wote/pi-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@handy_wote/pi-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@handy_wote/pi-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@handy_wote/pi-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@handy_wote/pi-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@handy_wote/pi-ai`.
