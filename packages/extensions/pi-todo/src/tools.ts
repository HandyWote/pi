...
const TodoDefinitionSchema = Type.Object({
	...
	acceptance_criteria: Type.Optional(Type.Array(Type.String({ minLength: 1 }))),
});
...
export function registerTodoTools(pi: ExtensionAPI, runtime: TodoRuntime): void {
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			items: Type.Array(TodoDefinitionSchema, { minItems: 1 }),
			global_direction: Type.Optional(Type.String({ minLength: 1 })),
		}),
		executionMode: "sequential",
		async execute(_toolCallId, params) {
			const list = await runtime.add(params.items as TodoDefinition[], params.global_direction);
			return result(`Added ${params.items.length} task(s).`, list);
		},
	});
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params) {
			...
			return result(formatList(view, tasks), { ...view, tasks });
		},
	});
	...
	pi.registerTool({
		name: "todo_update",
		label: "Update Todo",
		description: "Update task content, dependencies, ownership, or public status",
		...
		parameters: Type.Object({
			...
			active_form: Type.Optional(Type.Union([Type.String(), Type.Null()])),
			depends_on: Type.Optional(Type.Array(TodoId)),
			acceptance_criteria: Type.Optional(Type.Union([Type.Array(Type.String({ minLength: 1 })), Type.Null()])),
			status: Type.Optional(
				Type.Union([Type.Literal("pending"), Type.Literal("in_progress"), Type.Literal("completed")]),
			),
			owner: Type.Optional(Type.Union([Type.String({ minLength: 1 }), Type.Null()])),
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params) {
			...
			return result(
				`Claimed ${params.id} for ${claim.task.owner}. Preserve the claim token when completing or releasing it.`,
				{
					...claim,
					metadata: {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params) {
			...
			return result(`Deleted ${params.id} and cleaned dependent edges.`, list);
		},
	});
}
