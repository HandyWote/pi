...
| Package | Purpose |
| --------- | --------- |
| `@handy_wote/pi-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@handy_wote/pi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@handy_wote/pi-tui` | TUI components for custom rendering |
...
Notification-only lifecycle events for blocking user-facing extension UI prompts. They fire around `ctx.ui.select()`, `ctx.ui.confirm()`, and `ctx.ui.input()` so host/status integrations can report "waiting for user" instead of just "running".
...
pi.on("ui_prompt_start", async (event, ctx) => {
  ...
  // event.kind: "select" | "confirm" | "input"
...
```typescript
import { isToolCallEventType } from "@handy_wote/pi-coding-agent";
...
```typescript
import { isBashToolResult } from "@handy_wote/pi-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@handy_wote/pi-coding-agent";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@handy_wote/pi-coding-agent";
...
```typescript
import { SessionManager } from "@handy_wote/pi-coding-agent";
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@handy_wote/pi-ai";
...
```typescript
import type { AutocompleteItem } from "@handy_wote/pi-tui";
...
```typescript
import { Box, Text } from "@handy_wote/pi-tui";
...
Dynamic providers can implement `refreshModels`. Pi calls it during model refresh, publishes the returned list synchronously through the provider, and passes the canonical credential/store/network/signal context. The extension decides whether to persist the catalog through `context.store`; live servers can ignore it.

Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@handy_wote/pi-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@handy_wote/pi-ai";
...
// Register a live local-server catalog without persisting discovered models
...
```typescript
import { withFileMutationQueue } from "@handy_wote/pi-coding-agent";
...
**Important:** Use `StringEnum` from `@handy_wote/pi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@handy_wote/pi-coding-agent";
...
```typescript
import { createBashTool } from "@handy_wote/pi-coding-agent";
...
```typescript
import { keyHint } from "@handy_wote/pi-coding-agent";
...
```typescript
import { Text, Component } from "@handy_wote/pi-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@handy_wote/pi-coding-agent";
import { matchesKey } from "@handy_wote/pi-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@handy_wote/pi-coding-agent";
