...
- Changed runtime custom messages queued without a lane and with `triggerTurn: false` to append after the turn's tool results instead of steering mid-run.
...
- Removed the built-in `/llama` command and llama.cpp extension. Add local OpenAI-compatible servers (llama.cpp, Ollama, vLLM, LM Studio) as a Profile via `/profile` instead.
