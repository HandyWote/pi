...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@handy_wote/pi-ai`, `@handy_wote/pi-agent-core`, `@handy_wote/pi-coding-agent`, `@handy_wote/pi-tui`, `typebox`.
