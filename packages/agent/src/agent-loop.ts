	...
	while (true) {
		...
		while (hasMoreToolCalls || pendingMessages.length > 0) {
			if (lastCompletedTurn) {
				...
				// Events queued during long-running preparation are drained in full
				// (all-mode) so they are not delayed past this turn. The turn-end poll
				// above already drained earlier events; this only picks up new ones.
